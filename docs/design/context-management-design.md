...
role_content=$(cat "${PLUGIN_ROOT}/data/profile/role.md")
soul_content=$(cat "${PLUGIN_ROOT}/data/profile/soul.md")
...
def load_rule(rule_name):
    rule_path = f"rules/{get_rule_level(rule_name)}/{rule_name}.md"
    return read_file(rule_path)
```

**上下文影响**: ~200B (索引) + 按需加载 (~1KB)
