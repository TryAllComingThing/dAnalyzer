...
```
输出流程:
  data-query → data-analysis → (清洗) → 输出技能 → security → 输出
                                                              ↑
                                                      脱敏 + 合规检查 (强制)
```
...
2. **内嵌执行** — 需求拆解、任务规划、数据校验、结果格式化均为 danalyzer-core 内嵌能力，不再 spawn 独立 Agent
3. **按需调用** — 不预设固定流程
4. **条件触发** — 满足条件才执行拆解/规划步骤
5. **灵活组合** — 根据实际需求调整
6. **按需加载** — 只读取需要的 SKILL.md
...
2. **然后**内嵌执行需求拆解 (if needed) → 任务规划 (if needed)
3. **然后**按需加载 Skills 的 SKILL.md
4. **最后**按顺序执行 Skills

**唯一 spawn 例外**: 执行中发生异常时 → spawn `Agent: error-handler`

**错误做法**: 用户输入 → 直接执行 data-query → 输出结果
**正确做法**: 用户输入 → danalyzer-core → 需求拆解 (if needed) → 任务规划 (if needed) → 按需加载 Skills → 执行 → security → 输出
...
> error-handler 是 danalyzer-core 唯一 spawn 的独立 Agent。其他 Agent（demand-parse、task-planner、data-validator、result-formatter）的能力已内嵌到 danalyzer-core 自身。
