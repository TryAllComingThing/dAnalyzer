...
# 从文件读取，保存脱敏后的数据到文件
python scripts/security_scan.py --input knowledge/output.json --output knowledge/cleaned.json

# 仅检查不输出脱敏数据
python scripts/security_scan.py --check-only --input knowledge/output.json

# 显示详细扫描信息
python scripts/security_scan.py --input knowledge/output.json --verbose
```
