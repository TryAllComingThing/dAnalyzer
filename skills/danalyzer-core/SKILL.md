...
## 按需加载（Skills + Connectors）

仅在需要时读取对应的 SKILL.md 和 Connector 文档。数据 I/O 前必须读取对应的 connector 文档。
...
> data-clean 包含可选的**质量校验**步骤。当数据源不可靠、涉及关键业务决策、或用户明确要求质量报告时，在清洗后执行校验。
